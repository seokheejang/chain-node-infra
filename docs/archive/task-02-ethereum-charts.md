...
### 2026-04-09 gcMode, Ingress 컨벤션, PVC 사이즈 가이드

**상태**: 완료

#### 배경

Mainnet/Testnet 실운영 대비를 위해 archive/pruned 모드 지원, 외부 endpoint 노출 컨벤션, 모드별 PVC 사이즈 가이드를 추가.

#### 변경 내용

**1. gcMode (archive/pruned)**
- geth: `config.gcMode` first-class 값 추가 (`full`=pruned, `archive`=archive)
- `state.scheme` 자동 유도: full→path, archive→hash (사용자 실수 방지)
- lighthouse: `config.archiveMode` boolean + `--reconstruct-historic-states` 플래그

**2. Ingress nip.io 컨벤션**
- baremetal K8s용 `{service}.{ip}.nip.io` 패턴 예시
- per-port hostname 지원: string(기존 호환) + structured(`hostname:` + `port:`) 듀얼 포맷
- geth RPC/WS 포트별 분리, lighthouse 단일 포트
- Ingress + HTTPRoute 양쪽 동일 패턴 적용

**3. PVC 사이즈 가이드**
- geth/lighthouse values.yaml에 네트워크×모드별 권장 사이즈 테이블 주석
- geth v1.14+ PebbleDB + path scheme 기준 추산
- 환경 파일에 모드별 사이즈 가이드 주석 포함

**4. Environments 3-tier 구조**
- `environments/{tier}/{network}/` 계층: mainnet / testnet / private
- testnet 하위에 sepolia (향후 holesky 등 추가 가능)
- 신규: `environments/mainnet/lighthouse.yaml`, `environments/testnet/sepolia/`

| 파일 | 변경 |
|------|------|
| `charts/geth/values.yaml` | gcMode, PVC 가이드, ingress 예시 |
| `charts/geth/templates/statefulset.yaml` | --gcmode, --state.scheme args |
| `charts/geth/templates/ingress.yaml` | kindIs 듀얼 포맷 분기 |
| `charts/geth/templates/httproute.yaml` | kindIs 듀얼 포맷 분기 |
| `charts/geth/templates/NOTES.txt` | GC Mode + ingress URL |
| `charts/lighthouse/values.yaml` | archiveMode, PVC 가이드, ingress 예시 |
| `charts/lighthouse/templates/statefulset.yaml` | --reconstruct-historic-states |
| `charts/lighthouse/templates/ingress.yaml` | kindIs 듀얼 포맷 분기 |
| `charts/lighthouse/templates/httproute.yaml` | kindIs 듀얼 포맷 분기 |
| `charts/lighthouse/templates/NOTES.txt` | Archive Mode + ingress URL |
| `environments/mainnet/geth.yaml` | gcMode, size 600Gi, ingress 예시 |
| `environments/mainnet/lighthouse.yaml` | 신규 |
| `environments/testnet/sepolia/` | 신규 (geth + lighthouse) |
| `environments/private/geth.yaml` | gcMode: full 명시 |
| `environments/private/lighthouse.yaml` | archiveMode: false 명시 |

#### 검증

- Ralph self-review 1 round, 전체 PASS
- helm template: 기본값 + mainnet + private + sepolia + CI + e2e 모두 렌더링 통과
- gcMode full/archive 분기 정상
- ingress string/structured 듀얼 포맷 정상
- HTTPRoute per-port backendRef 정상

#### 핸드오프

- `enableServiceLinks: false` geth 차트 적용 검토
- Phase 2 (Testnet) ArgoCD Application 매니페스트 생성
- holesky 환경 파일 추가
- → CHANGELOG.md에 기록됨
